...
- [x] Record fresh action-time approval for the prepared 6.6-credit voice job; no music action exists or has been dispatched.
- [ ] Review every completed audio result before marking it candidate or approved.
- [ ] Record model, prompt, date, result ID, local filename, and SHA-256 for every used audio asset.
...
- [ ] Conform the edit to `SHOT_LIST.md` and the approved voice-over.
