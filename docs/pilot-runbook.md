...
The enrolled device is a passkey/security key. AWS supports it in the browser sign-in used by `aws login`, but not as the six-digit `mfa_serial` input for an MFA-protected API call. The role profile therefore intentionally omits `mfa_serial`. Sign out any console session created before MFA enrollment, run `aws login --profile stay-pilot-login`, authenticate through the browser with the passkey, and accept the setup only if the live role-profile identity check succeeds. If AWS does not propagate MFA context into the login-backed session, add a separately reviewed virtual or hardware TOTP device rather than weakening the role trust. See [AWS passkey support](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_mfa_fido_supported_configurations.html) and [AWS login](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sign-in.html).
