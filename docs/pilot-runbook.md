...
The prepared operator identity design uses a login-only IAM user named `stay-pilot-operator-login` and a one-hour role named `stay-pilot-household-operator-role`. Do not create access keys. Attach AWS managed `SignInLocalDevelopmentAccess` to the user so `aws login --profile stay-pilot-login` can obtain temporary credentials. The exact user policy is `infrastructure/iam/pilot-operator-login-policy.json`; it permits only the user's own password/MFA bootstrap and assumption of the exact operator role, while an explicit deny blocks other actions before MFA. The exact role trust is `infrastructure/iam/pilot-operator-role-trust.json`; it names that user and requires `aws:MultiFactorAuthPresent=true`. Attach only `arn:aws:iam::828547077857:policy/stay-pilot-household-operator` to the role. Configure `stay-pilot-operator` as an MFA role profile sourced from `stay-pilot-login`, verify that caller is `arn:aws:sts::828547077857:assumed-role/stay-pilot-household-operator-role/...`, and prove unrelated IAM/S3/deployment operations are denied before any operator apply.

The account has no IAM password policy and no IAM users before this setup. Apply a reviewed strong password policy before creating the login profile: minimum 20 characters; uppercase, lowercase, number and symbol required; prevent reuse of 24 prior passwords; no routine expiry. Create a one-time random password without printing it, require first-login reset, then enroll MFA. The user's self-service permissions must remain scoped to its own password and MFA device. The root login remains reserved for administration and read-only recovery inspection.

The forced first-login page can evaluate the permanent no-MFA deny before an operator has an MFA device. If CloudTrail reports an identity-policy explicit deny for `iam:ChangePassword`, temporarily replace the inline policy with `infrastructure/iam/pilot-operator-first-sign-in-policy.json`. This policy allows only the password/MFA bootstrap actions and the exact role assumption; the role trust still rejects assumption without MFA, while all unrelated actions remain implicitly denied. Immediately after password reset and MFA enrollment, restore `infrastructure/iam/pilot-operator-login-policy.json` and re-run the live deny simulations.
