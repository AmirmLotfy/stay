...
Required user/provider inputs are pending: a scoped non-root AWS profile (only the root-backed default profile was found), participant full video review and Devpost entry/submission, and actual pilot consent/verified identities. After deployment, complete live authenticated isolation/MCP/WS, inbox/feedback/alert, restore/rollback and device accessibility checks before enrollment. The actual seven-day and fourteen-day observation gates remain unchanged.
