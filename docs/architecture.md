...
- SES sends minimal messages as `STAY <updates@saystay.site>`. The domain uses Easy DKIM 2048, a custom `mail.saystay.site` MAIL FROM domain with SPF, and DMARC. SES accepted the live sender test, the transactional production-access request, and the requested follow-up in case `178838582000594`; the last verified API state remains `DENIED` with `ProductionAccessEnabled=false`, so the sandbox remains active while Support reviews the reply. Inbox confirmation remains separate. Sensitive detail stays inside an authenticated, active, assigned incident.
